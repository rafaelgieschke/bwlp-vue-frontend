...
3. (optional) style `<style scoped></style>
   ...
   - Use scoped styles in 99.9% of cases, if you need global CSS, put it in the main CSS file, if you have one of the specific cases where the CSS works weird within an SFC, use `:global` on that specific rule, not on the entirety of the SFC's CSS, and give it a very specific class so it's not applied to divs project-wide or something
   - Try learning about deep and slotted selectors
...
My (original developer of this particular web app) reasons are the following:
